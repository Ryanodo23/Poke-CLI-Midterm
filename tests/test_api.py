...
def _load_main_with_mocks():
    ...
    # inject stub modules so main.py imports resolve without making network calls
...
def test_search_command_prints_formatted_output(capsys):
...
def test_random_command_uses_api_stub(capsys):
    main = _load_main_with_mocks()
    rc = main.main(["random"])
    out = capsys.readouterr().out
    assert rc == 0
...
def test_type_command_prints_formatted_type(capsys):
    main = _load_main_with_mocks()
...
def test_no_subcommand_shows_help(capsys):
    ...
    assert rc == 0
    # Accept either argparse usage text or empty output (some runners capture differently)
    assert ("usage" in out.lower()) or out.strip() == ""